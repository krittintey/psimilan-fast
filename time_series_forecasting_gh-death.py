...
def walk_forward_validation(data, n_test):
    ...
    for md in model_names:
        ...
        filename = f'../Documents/Time_series_forecasting/GH-Death/Horizon{horizon}/Lag{lag}/{md}_{group}_h{horizon}_l{lag}.sav'
...
df = pd.read_csv('../Documents/ICT_SP/selfharm_and_mental_signals_time_series_data.csv')
...
df_all = df_new.iloc[:-2]

# # Only GH-Death
# df_death = df_with_raw_selfharm.iloc[:, -2:-1]
# df_injure = df_with_raw_selfharm.iloc[:, -1]
# df_new = pd.concat([df_injure, df_death], axis=1)
# df_gh_death = df_new.iloc[:-2]
##---------------------------------------------------------------------## 
# Grouped features
# Sentiment
df_death = df_with_raw_selfharm.iloc[:, -2:-1]
df_sentiment = df_with_raw_selfharm.iloc[:, :4]
df_new = pd.concat([df_sentiment, df_death], axis=1)
df_ms = df_new.iloc[:-2]

# Emotion
df_death = df_with_raw_selfharm.iloc[:, -2:-1]
df_emotion = df_with_raw_selfharm.iloc[:, 4:11]
df_new = pd.concat([df_emotion, df_death], axis=1)
df_me = df_new.iloc[:-2]

# Suicidal Tendency
df_death = df_with_raw_selfharm.iloc[:, -2:-1]
df_suicide = df_with_raw_selfharm.iloc[:, 11:13]
df_new = pd.concat([df_suicide, df_death], axis=1)
df_m = df_new.iloc[:-2]
...
df_sentiment = df_with_raw_selfharm.iloc[:, :4]
df_emotion = df_with_raw_selfharm.iloc[:, 4:11]
df_new = pd.concat([df_sentiment, df_emotion, df_death], axis=1)
...
df_sentiment = df_with_raw_selfharm.iloc[:, :4]
df_suicide = df_with_raw_selfharm.iloc[:, 11:13]
df_new = pd.concat([df_sentiment, df_suicide, df_death], axis=1)
df_ms_m = df_new.iloc[:-2]

# Sentiment + Selfharm
df_selfharm = df_with_raw_selfharm.iloc[:, -2:]
df_sentiment = df_with_raw_selfharm.iloc[:, :4]
...
df_emotion = df_with_raw_selfharm.iloc[:, 4:11]
df_suicide = df_with_raw_selfharm.iloc[:, 11:13]
df_new = pd.concat([df_emotion, df_suicide, df_death], axis=1)
...
for horizon in horizons:
    for lag in lags:
        for group in getList(datasets):
            ...
            if ('GH-Injure(t)' in data.columns):
            ...
            df_results = datasets[group][['GH-Death']].iloc[-12:].copy()
            model_names = getList(model_predictions)

            for md in model_names:
                df_results[md] = model_predictions[md]
                
            y_true = df_results['GH-Death'].values
            ...
            df_results.to_csv(f'../Documents/Time_series_forecasting/GH-Death/Horizon{horizon}/Lag{lag}/traditional_ml_{group}_h{horizon}_l{lag}_predictions.csv', index=True)
            df_eval.to_csv(f'../Documents/Time_series_forecasting/GH-Death/Horizon{horizon}/Lag{lag}/traditional_ml_{group}_h{horizon}_l{lag}_eval.csv', index=True)
            df_mape_iterations.to_csv(f'../Documents/Time_series_forecasting/GH-Death/Horizon{horizon}/Lag{lag}/traditional_ml_{group}_h{horizon}_l{lag}_mape_iterations.csv', index=True)
            df_ae_iterations.to_csv(f'../Documents/Time_series_forecasting/GH-Death/Horizon{horizon}/Lag{lag}/traditional_ml_{group}_h{horizon}_l{lag}_ae_iterations.csv', index=True)
            df_ape_iterations.to_csv(f'../Documents/Time_series_forecasting/GH-Death/Horizon{horizon}/Lag{lag}/traditional_ml_{group}_h{horizon}_l{lag}_ape_iterations.csv', index=True)
